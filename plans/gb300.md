...
**Blocked on:** [arm-image.md](./arm-image.md) (land/stack that PR first)  
**Out of scope:** GB200 (separate plan/PR), multi-node NCCL, Arm Dockerfile contents  
**Follow-up SKU:** `Standard_ND128isrG5_GB300_v6` (second conf once primary works)

---

## 1. Goals

Single-node Az NHC parity on **ND GB300 v6** using the **existing aarch64 NVIDIA image** from the arm-image PR.

1. Auto-detect SKU → `conf/nd128isr_gb300_v6.conf`
2. Presence checks: CPU, mem, NVMe, eth, IB, GPU health
3. Perf floors: GPU BW, NCCL allreduce, IB GDR
4. Topology, clock boost, AN/eth rate wiring, tests, docs

Success criteria:

- [ ] `sudo ./run-health-checks.sh` exits 0 on healthy GB300
- [ ] `-a -v` has no NHCNA tooling/SKU-unsupported faults
- [ ] Unit/integration helpers know GB300
- [ ] README lists GB300 + calibrated thresholds

Non-goals: multi-node NCCL; GB200; reworking the Arm image (fix only if a binary is missing).
...
### WS1 — aarch64 NVIDIA container image (blocking) → **separate PR**

Tracked in [`arm-image.md`](./arm-image.md). Do not re-implement here.

- [ ] Arm image PR merged (or stacked branch available)
- [ ] Runner selects arm image on this host
- [ ] Smoke: `nvidia-smi`, nccl-tests, `ib_write_bw` inside container

**Exit criteria:** image works on Grace before any GB300 conf merge.
...
## 5. Suggested PR commit plan (GB300 SKU PR only)

Assumes arm-image PR already landed. Keep commits bisectable.

1. `feat(conf): add nd128isr_gb300_v6 presence-check conf`  
   - conf with hardware/GPU presence; perf checks TODO or loose placeholders
2. `feat(runner): AN/eth rate handling for GB300 (e.g. 160G)`  
   - only networking lists/logic; no image work
3. `feat(checks): GB300 topology + clock boost hooks`  
   - topo xml + `azure_hw_topology_check.nhc` + `boost_gpu_clock` if needed
4. `fix(checks): NCCL/IB GDR device layout for 4-GPU GB300`  
   - index mapping / env fixes discovered on hardware
5. `feat(conf): calibrated GB300 GPU/IB/NCCL thresholds`  
   - final conf numbers + README expected table
6. `test: GB300 sad-path conf and SKU helpers`  
   - `nhc-test-common.sh`, `bad_test_confs/`
7. `docs: document ND128isr_GB300_v6 support`  
   - README supported SKUs, developer_guide note
...
| WS | Status | Notes |
|----|--------|-------|
| WS0 Inventory | Not started | Confirm SKU on this aarch64 host |
| WS1 Arm image | Separate plan | [`arm-image.md`](./arm-image.md) |
| WS2 Conf + runner AN | Not started | |
| WS3 Check code | Not started | |
| WS4 Calibration | Blocked on WS1–3 | |
| WS5 Tests | Not started | |
| WS6 Docs | Plan drafted | |
| WS7 PR | Branch stub exists | `feature/gb300-v6-support` @ main |

---

## 11. Key code locations (SKU PR)

```
run-health-checks.sh                 # SKU detect, AN lists (not image — arm PR)
conf/nd128isr_gb300_v6.conf          # new
customTests/azure_common.nhc         # boost clocks, IB helpers
...
1. Finish/land [`arm-image.md`](./arm-image.md) implementation first.
2. WS0 inventory on GB300; fill §6.
3. Presence conf + check hooks; then calibrate perf.
